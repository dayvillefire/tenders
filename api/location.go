...
func init() {
	common.ApiMap["location"] = func(r *gin.RouterGroup) {
		r.PUT("", apiLocationSave)
		r.GET("/:id", apiLocationGet)
		r.DELETE("/:id", apiLocationDelete)
	}
	common.ApiMap["locations"] = func(r *gin.RouterGroup) {
		r.GET("", apiLocationList)
		r.GET("/:query", apiLocationList)
	}
}

func apiLocationSave(c *gin.Context) {
...
func apiLocationDelete(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
		c.AbortWithStatus(http.StatusBadRequest)
		return
...
func apiLocationGet(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
		c.AbortWithStatus(http.StatusBadRequest)
		return
...
func apiLocationList(c *gin.Context) {
	query := c.Param("query")
	pageNumber := c.Query("page")
	perPage := c.Query("perpage")
	var obj models.Locations
	var err error
	if query == "" {
